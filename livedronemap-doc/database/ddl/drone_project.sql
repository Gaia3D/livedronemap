...
comment on column drone_project.start_latitude is '촬영 시작 위도';
comment on column drone_project.start_longitude is '촬영 시작 경도';
comment on column drone_project.end_latitude is '촬영 종료 위도';
comment on column drone_project.end_longitude is '촬영 종료 경도';
comment on column drone_project.status is '상태. 0:준비중, 1:점검/테스트, 2:개별영상, 3:후처리영상 , 4:프로젝트 종료, 5:에러';
comment on column drone_project.shooting_date is '수정일';
